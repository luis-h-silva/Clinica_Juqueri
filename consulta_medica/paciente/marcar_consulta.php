...
if ($res_med = $mysqli->query($sql_med)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['medico_id'], $_POST['data_consulta'], $_POST['hora_consulta'])) {
    ...
    $datahora_agendada_str = "{$data_consulta} {$hora_consulta}"; 
    ...
    if ($stmt_conflito = $mysqli->prepare($sql_conflito)) {
        $stmt_conflito->bind_param("iss", $medico_id, $data_consulta, $hora_consulta); 
        ...
        if ($res_conflito->num_rows > 0) {
            ...
            if ($dados_conflito['paciente_id'] == $paciente_id) {
                ...
                $mensagem = '<p class="mensagem-erro">O horário selecionado ('.date('d/m/Y H:i', strtotime($datahora_agendada_str)).') não está mais disponível para este médico. Escolha outro horário.</p>';
            }
            $stmt_conflito->close();
            ...
            $sql_insert = "INSERT INTO consultas (paciente_id, medico_id, data, hora, status) 
                           VALUES (?, ?, ?, ?, 'ATIVA')";
            
            if ($stmt_insert = $mysqli->prepare($sql_insert)) {
                $stmt_insert->bind_param("iiss", $paciente_id, $medico_id, $data_consulta, $hora_consulta);
                
                if ($stmt_insert->execute()) {
                    $mensagem = '<p class="mensagem-sucesso">Sua consulta foi agendada com sucesso para '.date('d/m/Y H:i', strtotime($datahora_agendada_str)).'!</p>';
                ...
                $mensagem = '<p class="mensagem-erro">Erro na preparação da inserção da consulta: ' . $mysqli->error . '</p>';
            }
        }
    } else {
        $mensagem = '<p class="mensagem-erro">Erro na preparação da validação de conflito: ' . $mysqli->error . '</p>';
    }
}
?>
<!DOCTYPE html>
